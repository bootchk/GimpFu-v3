...
def get_colourlist(x, l):
    ...
    (c, n, sub_boxes) = x
    if n > 0:
        colourlist.append(n2rgb(c, l))
    for box in sub_boxes:
        colourlist += get_colourlist(box, l + 1)
    return colourlist
...
def loss(x, lo, parent, l):
    ...
    (c, n, sub_boxes) = x
    z = n >> l
    if l > 0 and n > 0 and z < lo[0]:
        lo = [z, l, x, parent]
    for box in sub_boxes:
        lo = loss(box, lo, x, l + 1)
    return lo
...
def levels_params(Lambda, Sigma, c_hi):
    """Gets parameters to be used in gimp levels command."""
    alpha = [0, 0, 0]
    s = [0, 0, 0]
    for C in [R, G, B]:
        s[C] = Sigma[C] * (c_hi[C] / 255.0) ** Lambda[C]
    smax = max(s)
    for C in [R, G, B]:
...
def restore(image, drawable, gammatarg, sat_choice, save):
    ...
    if pdb.gimp_drawable_is_gray(drawable):
        pdb.gimp_message("Restore3.py does not work on greyscale images."
                         "Either use the levels command from the colors menu, or scan the image in colour "
                         "(not convert from greyscale to RGB), restore it and then convert the result to greyscale.")
        return
    ...
    LAMBDA2 = [lamr, lamg, lamb]
    SIGMA2 = [sigr, sigg, sigb]
    ...
    if sat_choice == 1:
        ...
        (mean, std_dev, median, pixels, count, percentile) = pdb.gimp_histogram(sat_draw, 0, 0, 255)
...
register(
  "python-fu_Restore3",
  "Restore faded slide",
  "Restores scanned photos that have deteriorated with age. "
  "Estimates the loss of dyes in the emulsion and restores the original values. "
  "The Light-Dark parameter allows some choice about the result. "
  "Some pictures are improved by having the saturation increased but this calculation is "
  "VERY slow and hence is optional",
