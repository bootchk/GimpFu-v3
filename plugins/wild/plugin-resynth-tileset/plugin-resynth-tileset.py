...
def draw_rect(l,x,y,w,h,color):
    pdb.gimp_image_select_rectangle(l.image,
                                    CHANNEL_OP_REPLACE,x,y,w,h)
    pdb.gimp_context_set_foreground(color)
    pdb.gimp_edit_fill(l, FOREGROUND_FILL)
...
def make_blob_output_pattern(tile_width, inset, outer_radius, inner_radius):
    ...
    img = gimp.Image(10*w,5*h,GRAY)
    l = gimp.Layer(img, "Base", img.width,img.height,GRAY_IMAGE,100,
                   NORMAL_MODE)
    img.add_layer(l)
    ...
    for tile, pos in tiles.items():
        ...
        if outer_radius > 0:
            if tile.top == BLANK and tile.left == BLANK:
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, cx-b,cy-b,outer_radius,outer_radius)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
                pdb.gimp_image_select_ellipse(img, CHANNEL_OP_INTERSECT,cx-b,cy-b,2*outer_radius,2*outer_radius)
                pdb.gimp_edit_fill(l, FOREGROUND_FILL)
            if tile.top == BLANK and tile.right == BLANK:
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, cx+b-outer_radius,cy-b,outer_radius,outer_radius)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
                pdb.gimp_image_select_ellipse(img, CHANNEL_OP_INTERSECT,cx+b-2*outer_radius,cy-b,2*outer_radius,2*outer_radius)
                pdb.gimp_edit_fill(l, FOREGROUND_FILL)
            if tile.bottom == BLANK and tile.right == BLANK:
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, cx+b-outer_radius,cy+b-outer_radius,outer_radius,outer_radius)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
                pdb.gimp_image_select_ellipse(img, CHANNEL_OP_INTERSECT,cx+b-2*outer_radius,cy+b-outer_radius*2,2*outer_radius,2*outer_radius)
                pdb.gimp_edit_fill(l, FOREGROUND_FILL)
            if tile.bottom == BLANK and tile.left == BLANK:
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, cx-b,cy+b-outer_radius,outer_radius,outer_radius)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
                pdb.gimp_image_select_ellipse(img, CHANNEL_OP_INTERSECT,cx-b,cy+b-outer_radius*2,2*outer_radius,2*outer_radius)
                pdb.gimp_edit_fill(l, FOREGROUND_FILL)
        ...
        if inner_radius > 0:
            ...
            if is_corner(tile.left, tile.top):
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, x,y,inset,inset)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, x+inset-inner_radius,y+inset-inner_radius,inner_radius,inner_radius)
                pdb.gimp_edit_fill(l, FOREGROUND_FILL)
                pdb.gimp_image_select_ellipse(img, CHANNEL_OP_INTERSECT,x+inset-inner_radius*2,y+inset-inner_radius*2,inner_radius*2,inner_radius*2)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
            if is_corner(tile.top, tile.right):
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, x+w-inset,y,inset,inset)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, x+w-inset,y+inset-inner_radius,inner_radius,inner_radius)
                pdb.gimp_edit_fill(l, FOREGROUND_FILL)
                pdb.gimp_image_select_ellipse(img, CHANNEL_OP_INTERSECT,x+w-inset,y+inset-inner_radius*2,inner_radius*2,inner_radius*2)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
            if is_corner(tile.right, tile.bottom):
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, x+w-inset,y+h-inset,inset,inset)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, x+w-inset,y+h-inset,inner_radius,inner_radius)
                pdb.gimp_edit_fill(l, FOREGROUND_FILL)
                pdb.gimp_image_select_ellipse(img, CHANNEL_OP_INTERSECT,x+w-inset,y+h-inset,inner_radius*2,inner_radius*2)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
            if is_corner(tile.bottom, tile.left):
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, x,y+h-inset,inset,inset)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
                pdb.gimp_image_select_rectangle(img, CHANNEL_OP_REPLACE, x+inset-inner_radius,y+h-inset,inner_radius,inner_radius)
                pdb.gimp_edit_fill(l, FOREGROUND_FILL)
                pdb.gimp_image_select_ellipse(img, CHANNEL_OP_INTERSECT,x+inset-inner_radius*2,y+h-inset,inner_radius*2,inner_radius*2)
                pdb.gimp_edit_fill(l, BACKGROUND_FILL)
...
def make_blob_output(source, source_map, output_map, map_weight, autism, neighbourhood, trys):
    ...
    def copy_tile(src, src_x, src_y, dest, dest_x, dest_y):
        pdb.gimp_image_select_rectangle(src.image, CHANNEL_OP_REPLACE,
                                        src_x*w,src_y*w,w,w)
    ...
    def synth_image(a, tile_horiz=False, tile_vert=False, show=False):
        ...
        i.add_layer(l)
        mi = gimp.Image(i.width,i.height,GRAY)
        m = gimp.Layer(mi, "Mask", mi.width, mi.height,GRAY_IMAGE)
        mi.add_layer(m)
